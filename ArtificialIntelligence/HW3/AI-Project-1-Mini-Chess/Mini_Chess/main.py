...
BUTTON_WIDTH = 40
BUTTON_HEIGHT = 40
PLAY_BUTTON_POS = (300, 380)
RESTART_BUTTON_POS = (300, 430)
TOGGLE_BUTTON_1_POS = (200, 380)
TOGGLE_BUTTON_2_POS = (150, 380)
TOGGLE_BUTTON_3_POS = (200, 430)
TOGGLE_BUTTON_4_POS = (150, 430)
...
WHITE_MAN = False
GAME_STARTED = False
AI_LAST_RESPONSE_TIME = None
AI_TOTAL_RESPONSE_TIME = 0.0
AI_RESPONSE_COUNT = 0
...
def drawButtons(WINDOW, GAME_STATE):
    ...
    drawTextMessage(WINDOW, "MoveCount", [10, 476], pygame.Color('darkmagenta'), 15)
    drawTextMessage(WINDOW, MOVE_COUNT, [120, 476], pygame.Color('red'), 15)

    previous_response = f"{AI_LAST_RESPONSE_TIME:.3f}s" if AI_LAST_RESPONSE_TIME is not None else "--"
    average_response = f"{(AI_TOTAL_RESPONSE_TIME / AI_RESPONSE_COUNT):.3f}s" if AI_RESPONSE_COUNT > 0 else "--"

    drawTextMessage(WINDOW, "Previous turn response time:", [10, 500], pygame.Color('darkmagenta'), 13)
    drawTextMessage(WINDOW, previous_response, [210, 500], pygame.Color('red'), 13)
    drawTextMessage(WINDOW, "Average response time:", [10, 522], pygame.Color('darkmagenta'), 13)
    drawTextMessage(WINDOW, average_response, [180, 522], pygame.Color('red'), 13)
    ...
    black_man = pygame.Color(
        'blue') if BLACK_MAN else pygame.Color('gainsboro')

    makeButton(WINDOW, TOGGLE_BUTTON_2_POS, BUTTON_WIDTH,
               BUTTON_HEIGHT, TEXT='AI', BTN_COLOR=black_ai, BTN_RADIUS=15)
    makeButton(WINDOW, TOGGLE_BUTTON_1_POS, BUTTON_WIDTH+50,
               BUTTON_HEIGHT, TEXT='HUMAN', BTN_COLOR=black_man, BTN_RADIUS=15)

    # ==> Toggle Button (White Selection)
    white_ai = pygame.Color('blue') if WHITE_AI else pygame.Color('gainsboro')
    white_man = pygame.Color(
        'blue') if WHITE_MAN else pygame.Color('gainsboro')

    makeButton(WINDOW, TOGGLE_BUTTON_4_POS, BUTTON_WIDTH,
               BUTTON_HEIGHT, TEXT='AI', BTN_COLOR=white_ai, BTN_RADIUS=15)
    makeButton(WINDOW, TOGGLE_BUTTON_3_POS, BUTTON_WIDTH+50,
               BUTTON_HEIGHT, TEXT='HUMAN', BTN_COLOR=white_man, BTN_RADIUS=15)
...
def drawTextMessage(WINDOW, TEXT, POSITION, TEXT_COLOR, font_size=17):
...
def drawUIStatus(WINDOW, GAME_STATE):
    ...
    if BLACK_AI == BLACK_MAN == WHITE_AI == WHITE_MAN == False:
        ...
        drawTextMessage(WINDOW, turn, [12, 560], pygame.Color('olivedrab4'))

    elif GAME_STATE.checkMate or GAME_STATE.staleMate:
        drawTextMessage(WINDOW, '', [80, 560], pygame.Color('white'))

    elif (BLACK_AI or BLACK_MAN) and (WHITE_AI or WHITE_MAN) and not GAME_STARTED:
        turn = 'Click Play to Start!'
        drawTextMessage(WINDOW, turn, [80, 560], pygame.Color('olivedrab4'))
        ...
        drawTextMessage(WINDOW, turn, [100, 560], pygame.Color('olivedrab4'))
...
def main():
    ...
    global AI_LAST_RESPONSE_TIME, AI_TOTAL_RESPONSE_TIME, AI_RESPONSE_COUNT
    ...
    black_ai_rect = pygame.Rect(
        TOGGLE_BUTTON_2_POS[0], TOGGLE_BUTTON_2_POS[1], BUTTON_WIDTH, BUTTON_HEIGHT)

    white_human_rect = pygame.Rect(
        TOGGLE_BUTTON_3_POS[0], TOGGLE_BUTTON_3_POS[1], BUTTON_WIDTH+50, BUTTON_HEIGHT)
    white_ai_rect = pygame.Rect(
        TOGGLE_BUTTON_4_POS[0], TOGGLE_BUTTON_4_POS[1], BUTTON_WIDTH, BUTTON_HEIGHT)
        ...
        if GAME_STARTED:
            if not humanPlayer:
                if not gameOver:
                    ...
                    AI_LAST_RESPONSE_TIME = time.perf_counter() - ai_start_time
                    AI_TOTAL_RESPONSE_TIME += AI_LAST_RESPONSE_TIME
                    AI_RESPONSE_COUNT += 1
